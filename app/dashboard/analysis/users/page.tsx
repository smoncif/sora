...
/**
 * Composant interne pour le bouton Quitter Focus fixe (adapté pour les utilisateurs)
...
/**
 * Page d'analyse des utilisateurs optimisée
...
export default function UserAnalysisPage() {
  ...
  // ⚡ Scroll géré directement dans useItemFocus pour de meilleures performances
  
  // 🔒 MÉMORISATION : Props partagées pour éviter les re-renders en boucle
  const sharedItemProps = React.useMemo(() => {
  ...
  const handleExportResults = React.useCallback(async () => {
    ...
    try {
      await exportResultsToExcel(
        ...
        { mode: 'users' }, // Spécifier le mode users
  ...
  // Handler pour la sauvegarde optimisé (MODE USERS)
        ...
        <Box>
            ...
            {!focusedItem && (
              <OverviewStatsSection
                mode="users"
                analysisResult={workflow.fileManager.state.analysisResult}
                totalItems={workflow.calculations.totalItems}
            ...
            {focusedItem && (
              <Box sx={{ mb: 4, textAlign: 'center' }}>
                ...
                >
                  Analyse détaillée de l'utilisateur sélectionné
                </Typography>
              </Box>
            )}

            {/* Analyses par utilisateur - VERSION OPTIMISÉE */}
            <ResultsSection
              mode="users"
              analysisResult={workflow.fileManager.state.analysisResult}
              ...
              sharedBusinessRoleProps={sharedItemProps}
